...
TEST_INDENT1_RAW_FILENAME = os.path.join(
    os.path.dirname(__file__), "indent_test1_raw.sh"
)
TEST_INDENT1_BEAUTIFIED_FILENAME = os.path.join(
    os.path.dirname(__file__), "indent_test1_beautified.sh"
)
TEST_FUNCSTYLES_RAW_FILENAME = os.path.join(
    os.path.dirname(__file__), "func_styles_raw.sh"
)
...
TEST_HEREDOC1_RAW_FILENAME = os.path.join(
    os.path.dirname(__file__), "heredoc_test1_raw.sh"
)
TEST_HEREDOC1_BEAUTIFIED_FILENAME = os.path.join(
    os.path.dirname(__file__), "heredoc_test1_beautified.sh"
)

TEST_NOFORMATTER1_RAW_FILENAME = os.path.join(
    os.path.dirname(__file__), "noformatter_test1_raw.sh"
)
TEST_NOFORMATTER1_BEAUTIFIED_FILENAME = os.path.join(
    os.path.dirname(__file__), "noformatter_test1_beautified.sh"
)

TEST_ELIF1_RAW_FILENAME = os.path.join(os.path.dirname(__file__), "elif_test1_raw.sh")
TEST_ELIF1_BEAUTIFIED_FILENAME = os.path.join(
    os.path.dirname(__file__), "elif_test1_beautified.sh"
)
...
class TestBeautysh(TestCase):
    ...
    def assertIdenticalMultilineStrings(self, expected, value):
        ...
        self.assertEqual(
            len(valuelines),
            len(expectedlines),
            "Wrong line count in actual value:\n{}".format(value),
        )
        for idx in range(0, len(expectedlines)):
            self.assertEqual(
                ...
                "Line {} is different!\nExpected {} chars:\n[{}]\nActual string is {} chars:\n[{}]".format(
                    idx + 1,
                    len(expectedlines[idx]),
                    self.get_string_highlighted_whitespaces(expectedlines[idx]),
                    len(valuelines[idx]),
                    self.get_string_highlighted_whitespaces(valuelines[idx]),
                ),
            )
    ...
    def verify_func_style(self, idx):
        ...
        # we expect no parsing error
        self.assertIdenticalMultilineStrings(
            expecteddata, result
        )  # we expect no change in formatting

    def verify_generic_with_func_style0(self, idx):
        testdata = self.read_file(TEST_GENERIC_TEST_RAW_FILENAME + str(idx) + "_raw.sh")
        expecteddata = self.read_file(
            TEST_GENERIC_TEST_RAW_FILENAME + str(idx) + "_beautified.sh"
        )
        bb = Beautify()
