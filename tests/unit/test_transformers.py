...
class TestChangeFunctionStyle:
    """Tests for change_function_style()"""

    def test_fnpar_to_fnonly(self):
        result = change_function_style("function foo() {", FNPAR, FNONLY)
        assert result == "function foo {"

    def test_fnpar_to_paronly(self):
        result = change_function_style("function foo() {", FNPAR, PARONLY)
        assert result == "foo() {"

    def test_fnonly_to_fnpar(self):
        result = change_function_style("function bar {", FNONLY, FNPAR)
        assert result == "function bar() {"

    def test_fnonly_to_paronly(self):
        result = change_function_style("function bar {", FNONLY, PARONLY)
        assert result == "bar() {"

    def test_paronly_to_fnpar(self):
        result = change_function_style("baz() {", PARONLY, FNPAR)
        assert result == "function baz() {"

    def test_paronly_to_fnonly(self):
        result = change_function_style("baz() {", PARONLY, FNONLY)
        assert result == "function baz {"

    def test_no_change_when_none_target(self):
        result = change_function_style("function foo() {", FNPAR, None)
        assert result == "function foo() {"

    def test_same_style_unchanged(self):
        result = change_function_style("function foo() {", FNPAR, FNPAR)
        assert result == "function foo() {"


class TestApplyVariableStyle:
    """Tests for apply_variable_style()"""

    def test_braces_style_simple_variable(self):
        result = apply_variable_style("echo $HOME", "braces")
        assert result == "echo ${HOME}"

    def test_braces_style_multiple_variables(self):
        result = apply_variable_style("echo $HOME $USER", "braces")
    ...
    def test_braces_style_with_underscore(self):
        result = apply_variable_style("echo $MY_VAR", "braces")
        assert result == "echo ${MY_VAR}"

    def test_braces_style_with_numbers(self):
        result = apply_variable_style("echo $VAR1 $VAR2", "braces")
    ...
    def test_braces_preserves_special_variables(self):
        ...
        result = apply_variable_style("echo $1 $@", "braces")
...
class TestEnsureSpaceBeforeDoubleSemicolon:
    """Tests for ensure_space_before_double_semicolon()"""

    def test_adds_space_when_needed(self):
        result = ensure_space_before_double_semicolon("foo;;")
        assert result == "foo ;;"
    ...
    def test_handles_multiple_semicolons(self):
        result = ensure_space_before_double_semicolon("foo;;bar;;")
